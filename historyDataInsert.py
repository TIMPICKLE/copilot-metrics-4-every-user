...
def generate_date_range(start_date_str, end_date_str):
    """
    Generate a list of dates between two given dates (inclusive), formatted as 'YYYY-MM-DD'.
...
def main(today_date):
    ...
    metrics_file_path = os.path.join("/home/sysadm/logs", 'metrics', f'copilot-usage_{today_date}.json')
...
if __name__ == "__main__":
    while True:
        date_range = generate_date_range("2025-03-20", "2025-06-10")[::-1]  # Reverse the order to process from latest to earliest
        for item in date_range:
            logger.info(f"Processing data for date: {item}")
            try:
                main(item)
